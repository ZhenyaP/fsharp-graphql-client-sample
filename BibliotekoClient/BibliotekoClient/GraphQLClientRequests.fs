...
module GraphQLClientRequests =
    ...
    type RegistriResponse = JsonProvider<"getregistri_response.json">
    type RegistrisResponse = JsonProvider<"getregistris_response.json">
    type BibliotekosResponse = JsonProvider<"getbibliotekos_response.json">
    type AddPetskriboResponse = JsonProvider<"addpetskribo_response.json">
    ...
    let asyncAddPetskriboToBiblioteko (bibliotekoID: System.Guid) (petskribo : InputPetskribo) : Async<Biblioteko> =    
        async {
            // Dispose the connection after using it.
            use connection = new GraphQLClientConnection()
            let request : GraphQLRequest =
                { Query = """
                mutation m($bibliotekoID: String!, $petskribo: InputPetskribo!, $UserId: String!) {
                    addPetskribo(bibliotekoID: $bibliotekoID, petskribo: $petskribo, UserId: $UserId) {
                        id
                        content {
                            id
                            registri {
            ...
            let biblioteko = 
                addPetskriboResponse.Data.AddPetskribo |> 
                        fun b -> 
                           {    
                                Id = b.Id
                                Address = 
